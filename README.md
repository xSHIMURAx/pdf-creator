...
## Cambio mas importante: ya no depende de ImageMagick

Toda la conversion de imagenes (lectura de JPG/PNG/WEBP/AVIF/BMP, miniaturas,
normalizacion a JPEG con la calidad elegida) ahora la hace `sharp`
(libvips), que es una dependencia npm con binarios precompilados — no un
`.exe` externo que haya que descargar, instalar o mandar por separado. Esto
resuelve de raiz el problema de los falsos positivos de antivirus que daba
`ConvertidorPDF.exe` compilado con `ps2exe`: ya no hay un instalador
descargado y ejecutado en silencio en tiempo de ejecucion, ni un stub .NET
generico — es una app Electron comun, con la misma huella que miles de apps
...
## Funciones que quedaron afuera (a diferencia de la version PowerShell)

- Tema claro-oscuro automatico segun Windows: por ahora sigue siendo tema
  oscuro fijo (se puede agregar con `nativeTheme` de Electron si hace falta).
- Rutas ultra largas (>260 caracteres) en Windows: Node.js las maneja
  razonablemente bien en Windows 10/11 modernos sin el truco `\\?\` que
  usaba la version PowerShell, pero no se probo a fondo.
